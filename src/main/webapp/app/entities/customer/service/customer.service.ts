...
type RestOf<T extends ICustomer | NewCustomer> = Omit<T, 'dob' | 'createdDate' | 'travelDate'> & {
  dob?: string | null;
...
export class CustomerService {
  ...
  protected convertDateFromClient<T extends ICustomer | NewCustomer | PartialUpdateCustomer>(customer: T): RestOf<T> {
    return {
      ...customer,
      dob: customer.dob?.toJSON() ?? null,
      createdDate: customer.createdDate?.format(DATE_FORMAT) ?? null,
      travelDate: customer.travelDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCustomer: RestCustomer): ICustomer {
    return {
      ...
      travelDate: restCustomer.travelDate ? dayjs(restCustomer.travelDate) : undefined,
    };
  }
